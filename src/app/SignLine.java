...
public class SignLine extends JFrame {
	private static final long serialVersionUID = -5479597998710277782L;
	...
	public void paint(Graphics g) {
		...
		g.drawString("y = " + df.format(cx) + " * sin( " + df.format(cy)
				+ " * x)", 50, 50);
		...
		for (double i = -Math.PI / cx; i < Math.PI / cx; i += 0.1)
			g.drawLine(translateX(i), translateY(sin(i)), translateX(i + 0.1),
					translateY(sin(i + 0.1)));
	}
